...
MuscleForceDirection::MuscleForceDirection(Model* aModel)
        : Analysis(aModel),
          ...
          _printAttachPoints(_boolPrintAttachPointProp.getValueBool()),
          _bodyNames(_strArrayBodyProp.getValueStrArray()),
...
// COPY CONSTRUCTOR
MuscleForceDirection::MuscleForceDirection(
        const MuscleForceDirection& aMuscleForceDirection)
        : Analysis(aMuscleForceDirection),
          _expressInLocalFrame(_expressInLocalFrameProp.getValueBool()),
          _printAttachPoints(_boolPrintAttachPointProp.getValueBool()),
          _bodyNames(_strArrayBodyProp.getValueStrArray()),
          _effectInsertion(_boolEffectiveInsertionsProp.getValueBool()) {
...
//============  OPERATORS  ============
// ASSIGNMENT: Assign this object to the values of another.@return Reference to
// this object.

MuscleForceDirection& MuscleForceDirection::operator=(
        const MuscleForceDirection& aMuscleForceDirection) {
    // Base Class
...
void MuscleForceDirection::setupProperties() {
    ...
    _expressInLocalFrameProp.setComment(
            "Flag (true or false)indicating whether the results will be "
            "express in the segment local reference systems or not.");
    _propertySet.append(&_expressInLocalFrameProp);

    _boolEffectiveInsertionsProp.setName("effective_attachments");
    _boolEffectiveInsertionsProp.setComment(
            "Flag (true or false) specifying whether the muscle force "
    ...
    _strArrayBodyProp.setComment("Names of the bodies whose attached muscles "
    ...
    _strArrayMuscleProp.setComment("Names of the muscles to be considered in the
    analysis." "The key word 'auto' indicates that the analysis will be
    performed on the muscles attached to the specified bodies.");
...
void MuscleForceDirection::constructDescription() {
    ...
    if (_expressInLocalFrame) {
        descrip = "\nThis file contains the normalized muscle lines of actions "
                  "expressed in the segment reference system.";
    } else {
        descrip = "\nThis file contains the normalized muscle lines of actions "
                  "expressed in the global reference system.";
    }
    descrip += "\n The muscle attachments (anatomical or effective) can be "
               "printed in a separate file if desired.";
    descrip += "\n";
...
void MuscleForceDirection::constructDescriptionAttachments() {
    ...
    if (_expressInLocalFrame) {
        descripAttach += "\n The positions are expressed in the segment "
                         "reference system.";
    } else {
        descripAttach += "\n The positions are expressed in the global "
                         "reference system.";
    }
...
void MuscleForceDirection::constructColumnLabels() {
    ...
    // labels in LOCAL (segment)reference frame
    {
        for (int i = 0; i < _muscleIndices.getSize(); i++) {
            ...
            auto FirstBodyName = pathpointSet[0].getParentFrame().getName();
            auto LastBodyName =
                    pathpointSet[SetLast].getParentFrame().getName();
...
int MuscleForceDirection::record(const SimTK::State& s) {
    ...
    for (int i = 0; i < _muscleIndices.getSize(); i++) {
        ...
        The latter gets the point of the path (presumably from getCurrentPath)
        and express them in LOCAL reference system of the body where they are
        connected.
        ------------------------------------------------------------ */
        ...
        //-----  TO DO: improve EffectiveAttachments in order to include this
        // selection ----------------

        // Selection of attachments to consider: anatomical (false) or effective
        // (true). The attachment and following point are selected to identify
        // the direction.
        ...
        } else { // Indexes identify as described in the body of function
                 // (below).
        ...
        // NB: if line "AttachIndex.setSize(4);" was included I should have used
        // .set(). TOOK A WHILE TO SPOT THE ERROR
        ...
        for (int ind = 0; ind < AttachIndex.getSize(); ind++) {
            // The position of the points extracted from PFD are expressed in
            // the global ref system.
            int n_att = AttachIndex.get(ind);
            // _model->getSimbodyEngine().transformPosition(
            //         s, PFDs[n_att]->frame(), PFDs[n_att]->point(), pos);
            pos = PFDs[n_att]->frame().findStationLocationInGround(
                    s, PFDs[n_att]->point());
            GlobPos.set(ind, pos);
        }
        ...
        //------------- TO DO: avoid assignation through a conditional sentence
        // when not required----
        ...
        // If requested, transform in local coordinate system the global muscle
        // lines of action
        if (_expressInLocalFrame == true) {
            // transforming muscle directions in local reference system
            // _model->getSimbodyEngine().transform(s, _model->getGround(), dir1,
            //                                      PFDs[FirstIndex]->frame(),
            //                                      dir1);
            dir1 = _model->getGround().expressVectorInAnotherFrame(
                    s, dir1, PFDs[FirstIndex]->frame());
            // _model->getSimbodyEngine().transform(s, _model->getGround(), dir2,
            //                                      PFDs[LastIndex]->frame(),
            //                                      dir2);
            dir2= _model->getGround().expressVectorInAnotherFrame(
                    s, dir2, PFDs[LastIndex]->frame());
        ...
        // copying the data from the direction vectors to the storage through
        // _muscledir
        int I = 6 * i;
        ...
        // (if required) copying the data from the position vectors to the
        // storage through _attachpointpos
        if (_printAttachPoints) {
            memcpy(&_attachpointpos[I], &pos1[0], 3 * sizeof(double));
            memcpy(&_attachpointpos[I + 3], &pos2[0], 3 * sizeof(double));
            _storeAttachPointPos.append(s.getTime(), _attachpointpos.getSize(),
                                        &_attachpointpos[0]);
        }
...
int MuscleForceDirection::begin(SimTK::State& s) {
    ...
    if (_printAttachPoints) { _storeAttachPointPos.reset(s.getTime()); }
 ...
 */
int MuscleForceDirection::printResults(const string& aBaseName,
                                       const string& aDir, double aDT,
                                       const string& aExtension) {
    // Print muscle directions
    Storage::printResult(&_storeDir, aBaseName + "_" + getName() + "_vectors",
                         aDir, aDT, aExtension);

    // Print muscle attachments
    if (_printAttachPoints) {
        Storage::printResult(&_storeAttachPointPos,
                             aBaseName + "_" + getName() + "_attachments", aDir,
                             aDT, aExtension);
    }
...
// IsMuscleAttachedToBody:  the function tells if a given Muscle is connected to
// a Body (specified by a BodyName)
bool MuscleForceDirection::isMuscleAttachedToBody(const Muscle& aMuscle,
    ...
    if (aPointSet[0].getParentFrame().getName() == aBodyName ||
        aPointSet[FinalPointIndex].getParentFrame().getName() == aBodyName) {
        Attached = true;
    }
...
// Given a pointer to a model and a string array of Body names, the indexes of
// the attached muscles are returned.
Array<int> MuscleForceDirection::getMusclesIndexForBody(
    ...
    if (bodyNames[0] == "all") {
        ...
        for (int j = 0; j < muscles.getSize(); j++) musclesIndexForBody[j] = j;
    }
    ...
    // CASE 2: choice of subset of segments
        ...
        for (int n_body = 0; n_body < bodyNames.getSize(); ++n_body) {
            ...
            checking all the muscles versus all the bodies contained in the
            Bodyset. NB: The array containes double indexes.
            ------------------------------------------------------------------*/

            for (int n_mus = 0; n_mus < muscles.getSize(); n_mus++) {
                if (isMuscleAttachedToBody(muscles[n_mus], aBodyName)) {
                    MusclesIndexForBody_temp.set(
                            k, muscles.getIndex(muscles[n_mus].getName()));
                    k++;
                }
            }
        }
        ...
        /*----------------NOTE--------------------------------------------------
        The previously created array of muscle indexes MusclesIndexForBody_temp
        is modified in order to contained unique indexes. The check consists in
        comparing an index with the following indexes. If no index in the
        following components is found then the index is saved. Each muscle is
        counted only once here!
        ----------------------------------------------------------------------*/
        // TODO: change this using std::unique

        for (int n_ind = 0; n_ind < MusclesIndexForBody_temp.getSize();
             n_ind++) {
            bool check = false;
            for (int n_ind2 = n_ind + 1;
                 n_ind2 < MusclesIndexForBody_temp.getSize(); n_ind2++) {
                if (MusclesIndexForBody_temp[n_ind] ==
                    MusclesIndexForBody_temp[n_ind2]) {
                    check = true;
                    break;
                }
            }

            // The check consists in comparing an index with the following
            // indexes. If no index in the following components is found then
            // the index is saved.
            if (!check) {
...
void MuscleForceDirection::getEffectiveAttachments(
        const Array<PointForceDirection*>& aPFDs, int& effecInsertProx,
        int& effecInsertDist) {
